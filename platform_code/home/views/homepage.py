...
from home.models import Membership, PendingInvitation, User, UserResources
    ...
    def login(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            context["message_login"] = {
                "alert-warning": _(
                    "The attempt to connect with this combination email/password failed. Please try again!"
                )
            }
    ...
    def signup(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            message = render_to_string(
                "home/account/acc_activate_email.html",
                {
                    "user": user,
...
def activate(request, uidb64, token):
    ...
    try:
        ...
        logger.error(
            f"[account_activation_error] The user with uidb64 {uidb64} does not exist, error {e}."
        )
    try:
        ...
        token_decoded = None
        logger.error(
            f"[account_activation_error] There is an issue to decode the token for the request {request},"
    ...
    if user is not None and token_decoded == user.email:
        ...
        if PendingInvitation.get_list_pending_invitation(user):
            ...
            logger.info(
                f"[account_creation][join_organisation] The user {user.email} created an account and has "
        ...
        logger.error(
            f"[account_activation_error] The activation of the account failed, request {request}"
            f", uidb64 {uidb64}"
        )
        return redirect("home:homepage")
