...
from assessment.views.utils.error_handler import error_500_view_handler, error_400_view_handler
...
class ProfileView(LoginRequiredMixin, generic.DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        success_max_points = manage_evaluation_max_points(request=request, evaluation_list=self.context["evaluations"])
    ...
    def add_organisation_list_context(self, user):
        ...
        organisation_list = []
        # Can be empty query
        query_membership = Membership.objects.filter(user=user)  # Possibility to add a condition on the role
