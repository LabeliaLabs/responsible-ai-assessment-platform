...
class OrganisationAdmin(admin.ModelAdmin):
    ...
    def export_data_to_csv(self, request, queryset):
        ...
        response[
            "Content-Disposition"
        ] = f'attachment; filename="organisation-data-{today}.csv"'
        ...
        for organisation in queryset:
            for evaluation in organisation.evaluation_set.all():
                for section in evaluation.section_set.all().order_by(
                    "master_section__order_id"
                ):
                    for element in section.evaluationelement_set.all().order_by(
                        "master_evaluation_element__order_id"
                    ):
                        for choice in element.choice_set.all().order_by(
                            "master_choice__order_id"
                        ):
                            writer.writerow(
                                [
                                    organisation.name,
                                    ...
                                    evaluation.name,
                                    evaluation.id,
                                    evaluation.assessment.version,
                                    evaluation.created_by,
                                    evaluation.is_finished,
                                    evaluation.finished_at,
                                    section.master_section.get_numbering(),
                                    section.fetch,
                                    section.user_notes,
                                    element.master_evaluation_element.get_numbering(),
                                    element.is_applicable(),
                                    element.fetch,
                                    element.user_notes,
                                    choice.master_choice.get_numbering(),
                                    choice.fetch,
                                    choice.is_ticked,
                                ]
                            )
