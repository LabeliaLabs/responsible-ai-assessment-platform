...
from assessment.views.utils.utils import treat_delete_note
...
class NotesTestCases(TestCase):
    ...
    def test_treat_delete_note(self):
        ...
        request = self.factory.post("/fr/accounts/profile/", {"note_element_id": self.evaluation_element2.id})
        ...
        request = self.factory.post("/fr/accounts/profile/", {"note_element_id": self.evaluation_element1.id})
        request.user = self.user
        response = treat_delete_note(request)
        self.evaluation_element1.refresh_from_db()
