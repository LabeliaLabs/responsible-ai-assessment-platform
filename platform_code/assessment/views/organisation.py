...
from home.models import Membership, Organisation, PendingInvitation, User
...
def leave_organisation(request, **kwargs):
    ...
    try:
        ...
        capture_message(
            f"[member_issue] The user {user.email} wants to leave the organisation {organisation.name},"
    ...
    if member:
        ...
        if member.role == "admin" and organisation.count_admin_members() == 1:
            messages.warning(
                request,
                _(
                    "You cannot leave the organisation %(organisation_name)s because you are"
                    " the unique admin member."
                )
                % {"organisation_name": organisation.name},
            )
            logger.info(
                f"[user_left_organisation_refused] The user {user.email} wanted to leave the organisation "
            ...
            if member.user.staff:
                ...
                member.delete()
                logger.info(
                    f"[user_left_organisation] The user {user.email} left the organisation {organisation.name} "
                    f"(id {organisation.id}) in which he was member as {member_role}."
                )
            messages.success(
                request,
                _("You are no longer member of the organisation %(organisation_name)s.")
                % {"organisation_name": organisation.name},
            )
...
class SummaryView(LoginRequiredMixin, DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        success_max_points = manage_evaluation_max_points(
            request=request, evaluation_list=self.context["evaluation_list"]
        )
        ...
        self.context["edit_organisation_form"] = OrganisationEditionForm(
            organisation=organisation, prefix="edit-organisation"
        )
    ...
    def add_evaluation_list_to_context(self, organisation):
        ...
        """
        self.context["evaluation_list"] = list(
            Evaluation.objects.filter(organisation=organisation).order_by("-created_at")
        )
    ...
    def add_member_data_and_forms_to_context(self, organisation):
        ...
        self.context["pending_member_list"] = PendingInvitation.get_organisation_pending_list(
            organisation
        )
        ...
        for member in self.context["member_list"]:
            self.context["edit_member_role_form_dic"][str(member.id)] = EditRoleForm(
                member=member, auto_id=False
            )
        ...
        for invitation in self.context["pending_member_list"]:
            self.context["edit_invitation_role_form_dic"][str(invitation.id)] = EditRoleForm(
                invitation=invitation, auto_id=False
            )
    ...
    def treat_add_member(self, request, organisation, user):
        ...
        if form.is_valid():
            ...
            if len(list_user_invited) == 1:
                ...
                if (
                    organisation.check_user_is_member(user=user_invited)
                    and not user_invited.staff
                ):
                    capture_message(
                        f"[add_member_failed] Invitation to join an organisation failed, "
                    ...
                    self.data_update["message"] = _(
                        "The user is already member. Please, " "edit his rights instead."
                    )
                ...
                if user_invited.staff:
                    self.add_staff_user_to_the_organisation(
                        organisation, user, user_invited, role
                    )

                # Case the user exists and he his not a member of the organisation
                if (
                    not organisation.check_user_is_member(user=user_invited)
                    and not user_invited.staff
                ):
            ...
            # No token or uid required as the link is just a link to signup
                ...
                self.send_invitation_to_join_organisation(
                    request, organisation, user, email_address, role
                )
        # Invalid form
        else:
    ...
    def add_staff_user_to_the_organisation(self, organisation, user, user_invited, role):
        ...
        # the staff user left the organisation
            ...
            Membership.create_membership(
                user=user_invited, organisation=organisation, role=role, hide_membership=False
            )
    ...
    def add_user_to_the_organisation(self, organisation, user, user_invited, role):
        ...
        """
        Membership.create_membership(user=user_invited, organisation=organisation, role=role)
        self.messages_user_added(organisation, user, user_invited, role)
    ...
    def messages_user_added(self, organisation, user, user_invited, role):
        ...
        """
        logger.info(
            f"[add_member_organisation] The user {user.email} has invited "
        ...
        self.data_update["data_user"] = {
            "0": user_invited.get_full_name(),
            "1": user_invited.email,
            "2": role,
            "3": "",
        }  # User can not already have created evaluations

    def send_invitation_to_join_organisation(
        self, request, organisation, user, email_address, role
    ):
        """
        ...
        message = render_to_string(
            "assessment/organisation/member/add-member-email.html",
            {
                "user": user,
    ...
    def messages_invitation_send(self, request, organisation, email_address, role):
        ...
        self.data_update["message"] = _(
            "The user does not yet have an account on the platform. "
            "An invitation has been sent to him."
        )
        logger.info(
            f"[add_member_invitation_sent] The user {request.user.email} has sent an email "
    ...
    def treat_remove_user(self, request, organisation, user):
        ...
        """
        member = get_object_to_action(
            request, organisation, user, Membership, "member", "remove"
        )
        if member:
            ...
            if not member.role == "admin" and member.organisation == organisation:
                ...
                # If the user is not staff, we just delete its membership
                    ...
                    member.delete()
                    logger.info(
                        f"[member_removed] The user {user.email} removed the membership of the user"
                ...
                self.data_update["success"] = True
                self.data_update["message"] = _(
                    "The user %(member_deleted_email)s is no longer a member of the "
                    "organisation and cannot access to it."
                ) % {"member_deleted_email": member_deleted_email}
            # Member is an admin member
            else:
                self.data_update["message"] = _(
                    "You cannot remove an admin member from the organisation."
                )
    ...
    def treat_delete_invitation(self, request, organisation, user):
        ...
        """
        invitation = get_object_to_action(
            request, organisation, user, PendingInvitation, "invitation", "delete"
        )
        if invitation:
        ...
        self.data_update["success"] = True
        self.data_update["message"] = _(
            "The invitation to %(invitation)s has been deleted."
        ) % {"invitation": invitation_deleted_email}
        logger.info(
            f"[invitation_deleted] The user {user.email} deleted the invitation for the email"
    ...
    def treat_edit_membership_role(self, request, organisation, user):
        ...
        """
        member = get_object_to_action(
            request, organisation, user, Membership, "member", "edit", hide_membership=False
        )
        if member:
            ...
            if not member.role == "admin" and member.organisation == organisation:
                ...
                if Membership.check_role(new_role):
                    ...
                    self.data_update["success"] = True
                    self.data_update["message"] = _(
                        "The user %(member_email)s has now the %(new_role)s "
                        "rights in the organisation."
                    ) % {"member_email": member.user.email, "new_role": new_role}
                    logger.info(
                        f"[member_role_edited] The user {user.email} edited the role of the user"
                ...
                # Role is not a valid one
                    ...
                    capture_message(
                        f"[member_role_edited_error] The user {user.email} tried to edit the "
                ...
                self.data_update["message"] = _(
                    "You cannot change the rights of an admin member."
                )
    ...
    def treat_edit_invitation_role(self, request, organisation, user):
        ...
        """
        invitation = get_object_to_action(
            request, organisation, user, PendingInvitation, "invitation", "edit"
        )
        if invitation:
            new_role = request.POST.get("role")  # role or None
            # Check that the new role is a role defined by the class Membership
            if Membership.check_role(new_role):
                ...
                self.data_update["success"] = True
                self.data_update["message"] = _(
                    "The invitation to %(invitation_email)s to join the"
                    " organisation"
                    " is now with the %(new_role)s rights in the"
                    " organisation."
                ) % {"invitation_email": invitation.email, "new_role": new_role}

                logger.info(
                    f"[invitation_role_edited] The user {user.email} edited the role of the "
            ...
            # Role is not a valid one
                ...
                capture_message(
                    f"[invitation_role_edited_error] The user {user.email} tried to edit the "
    ...
    def treat_edit_organisation(self, request, organisation, user):
        ...
        if form.is_valid():
            ...
            logger.info(
                f"[organisation_edited] The user {user}, with the role {organisation.get_role_user(user)},"
            ...
            self.data_update["message"] = _(
                "An error occurred, please verify the data you entered."
            )
...
def get_object_to_action(
    request, organisation, user, object_class, object_name, action, **kwargs
):
    """
    ...
    try:
        ...
        capture_message(
            f"[{object_name}_{action}_error] The user {user.email} wants to {action} the "
