...
def leave_organisation(request, **kwargs):
    ...
    try:
        ...
        capture_message(f"[member_issue] The user {user.email} wants to leave the organisation {organisation.name},"
        ...
        if form.is_valid():
            ...
            if len(list_user_invited) == 1:
                ...
                if organisation.check_user_is_member(user=user_invited) and not user_invited.staff:
                    capture_message(f"[add_member_failed] Invitation to join an organisation failed, "
        ...
        if member:
            ...
            if not member.role == "admin" and member.organisation == organisation:
                ...
                # Role is not a valid one
                    ...
                    capture_message(f"[member_role_edited_error] The user {user.email} tried to edit the "
    ...
    def treat_edit_invitation_role(self, request, organisation, user):
        ...
        if invitation:
            ...
            # Role is not a valid one
                ...
                capture_message(f"[invitation_role_edited_error] The user {user.email} tried to edit the "
...
def get_object_to_action(request, organisation, user, object_class, object_name, action, **kwargs):
    ...
    try:
        ...
        capture_message(f"[{object_name}_{action}_error] The user {user.email} wants to {action} the "
